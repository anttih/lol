...
(define (read-next)
  (let ((token (next-token)))
    (case (car token)
        ((eof) #!eof)
        ((close-paren) (car token))
        ((close-curly) (car token))
        ((close-bracket) (car token))
        ((open-paren) (read-s 'close-paren))
        ((open-curly) (cons 'hash-table (read-s 'close-curly)))
        ((open-bracket) (cons 'vector (read-s 'close-bracket)))
        ((keyword) (cadr token))
        ((symbol) (cadr token))
        ((number) (cadr token))
        ((string) (cadr token))
